...
namespace Chaite.Core
{
    ...
    internal sealed class QueenSlimeStrategy : BossStrategyBase
    {
        ...
        private static bool CanReturn(CombatSnapshot s, TargetSnapshot queen, bool second, SupportSpan floor, int direction)
        {
            ...
            {
                var t = s.Threats[i];
                var nativeLance = EmpressLanceGeometry.RequiresSafetyModel(t);
                if (nativeLance &&
                    !EmpressLanceGeometry.ValidSnapshot(t)) return false;
                if (t.Kind != ThreatKind.Projectile || t.TimeLeft <= 0) continue;
                var ticks = Math.Min(ReturnGuardTicks, t.TimeLeft);
                if (nativeLance || t.Geometry != ThreatGeometry.Body)
                {
    ...
    {
        private static readonly int[] PhaseOneAttacks =
            { 2, 8, 6, 8, 5, 2, 8, 4, 8, 5 };
        private static readonly int[] PhaseTwoClassicAttacks =
            { 7, 2, 8, 5, 2, 6, 4, 8, 12 };
        private static readonly int[] PhaseTwoExpertAttacks =
            { 7, 2, 8, 11, 5, 2, 6, 4, 8, 12 };

        public EmpressStrategy() : base("empress-of-light", 1700, 850, 6.75f, true) { }
        ...
        public override BossDecision Evaluate(CombatSnapshot s, BossMemory m)
        {
            ...
            // change until native state 10 completes.
            var genuinelyEnraged = form >= 2;
            ...
            var horizontal = 0;
            var vertical = 0;
            var distance = lethalDayContract ? 1040f : 700f;
            var margin = lethalDayContract ? 260f : 96f;
            var loop = StableLoopDirection(s, t, m,
            ...
            {
                case 0:
                    phase = "initial-native-reposition-" + tick;
                    pattern = lethalDayContract ? BossPattern.CircleOrbit :
                        BossPattern.EllipseOrbit;
                    horizontal = strictHorizontal;
                    vertical = strictVertical;
                    break;
                case 1:
                    var next = NextAttack(second, expertSchedule, nextIndex);
                    phase = "reposition-before-" + AttackName(next) + "-" +
                        tick;
                    pattern = next == 8 || next == 9 ?
                        BossPattern.PerpendicularDashDodge :
                        BossPattern.EllipseOrbit;
                    // Native has already committed to the next fixed table
                    // entry, so reserve its lane during the inter-attack move.
                    if (next == 8 || next == 9)
                    {
                        // The 8/9 attack is a horizontal body charge. Running
                        // away along the same lane still leaves the player
                        // inside the Empress's contact box, and in the lethal
                        // daytime contract that is a one-shot. Step off the
                        // attack line vertically before the native state 8/9
                ...
                case 4:
                    ...
                    horizontal = lethalDayContract ? strictHorizontal :
                        AwayX(s.Player, t);
                    vertical = lethalDayContract ? strictVertical :
                        -PerpendicularY(s.Player, t);
                    break;
                case 5:
                ...
                case 6:
                    ...
                    if (lethalDayContract &&
                        Vec2.DistanceSquared(s.Player.Center, t.Center) <
                            900f * 900f)
                    {
                        horizontal = AwayX(s.Player, t);
                        vertical = PerpendicularY(s.Player, t);
                ...
                case 8:
                case 9:
                    phase = tick <= 40 ? "horizontal-dash-telegraph-" +
                        (40 - tick) : tick <= 90 ?
                        "horizontal-dash-committed-" + (90 - tick) :
                        "horizontal-dash-braking";
                    pattern = BossPattern.PerpendicularDashDodge;
                    var committedVelocity = tick <= 40 ?
                        new Vec2(state == 8 ? -1f : 1f, 0f) :
                        t.Velocity;
                    ChargeEscape(s, t, committedVelocity, out horizontal,
                        out vertical);
                    margin += 42f;
                    break;
                case 10:
                    phase = "phase-transition-" + tick;
                    pattern = BossPattern.EllipseOrbit;
                    distance = lethalDayContract ? 1040f : 760f;
                    horizontal = strictHorizontal;
                    vertical = strictVertical;
                    break;
                case 11:
                    phase = "expert-predictive-lance-array-" + tick;
                    pattern = BossPattern.ProjectileLanes;
                    horizontal = lethalDayContract ? strictHorizontal :
                        AwayX(s.Player, t);
                    vertical = lethalDayContract ? strictVertical :
                    ...
                    break;
                case 12:
                    phase = "phase-2-spiral-bolts-" + tick;
                    pattern = BossPattern.CircleOrbit;
                    horizontal = strictHorizontal;
                    vertical = strictVertical;
                    break;
                case 13:
                    return Departing(s, t, genuinelyEnraged);
                default:
                    return UnknownNativeState(s, t,
                        "unreviewed-native-attack-" + state);
            }
            ...
            if (lethalDayContract || dash)
            {
                ...
                // still allowed to choose another immediately safer candidate;
                // the strategy does not claim ownership of collision closure.
        ...
        private static bool ValidNativeAttack(int state, int tick,
            int attackIndex, bool second, bool expertSchedule,
            bool fixedExpertDifficulty, bool forTheWorthy)
        {
            // State 3 is an unused development branch in the pinned release;
            // no production attack table can enter it.
            if (state == 3 || (!second &&
            ...
            if (state == 13) return tick == 0 && attackIndex > 0;

            if (state == 0 && attackIndex != 0) return false;
            if (state != 0 && state != 1 && state != 10)
            {
                // AI_120 increments ai[2] immediately after choosing a table
                // entry. State 9 is the same state-8 entry rewritten from the
                // player's side at selection time; it is never itself stored
                // in either table.
                if (attackIndex <= 0) return false;
                var matchesCurrent = MatchesPreviousAttack(state, second,
                    expertSchedule, attackIndex);
                // On classic, flag2 follows ShouldEmpressBeEnraged. A day/night
                // transition does not abort the already selected attack, so a
                // P2 snapshot may legitimately match the other schedule until
                // that attack returns to state 1. Expert/Master never has this
                // ambiguity because flag2 is permanently true.
                var matchesOtherClassicSchedule = second &&
                    !fixedExpertDifficulty && MatchesPreviousAttack(state,
                        second, !expertSchedule, attackIndex);
                if (!matchesCurrent && !matchesOtherClassicSchedule)
                    return false;
            }
            ...
            switch (state)
            {
                ...
                case 1:
                    var reposition = second ? 20f : 45f;
                    if (forTheWorthy) reposition *= .5f;
                    duration = (int)Math.Ceiling(reposition);
                    break;
            ...
            // Snapshot hooks can observe the terminal value before the native
            // branch writes state 1; admit that one boundary tick only.
            return tick <= duration;
        }

        private static int StableLoopDirection(CombatSnapshot snapshot,
            TargetSnapshot target, BossMemory memory, float turnReserve)
        {
            if (memory.PreviousTargetKey != target.Key ||
                memory.OrbitDirection == 0)
            {
                ...
                var awayRoom = away > 0 ? snapshot.Arena.ClearanceRight :
                    snapshot.Arena.ClearanceLeft;
                var otherRoom = away > 0 ? snapshot.Arena.ClearanceLeft :
                    snapshot.Arena.ClearanceRight;
                memory.OrbitDirection = awayRoom + 120f >= otherRoom ? away :
                    -away;
            }
            var heldRoom = memory.OrbitDirection > 0 ?
                snapshot.Arena.ClearanceRight : snapshot.Arena.ClearanceLeft;
            var reverseRoom = memory.OrbitDirection > 0 ?
                snapshot.Arena.ClearanceLeft : snapshot.Arena.ClearanceRight;
            if (heldRoom < turnReserve && reverseRoom > heldRoom + 160f)
                memory.OrbitDirection *= -1;
            return memory.OrbitDirection;
        }
        ...
        private static bool TryGetNativeCombat(CombatSnapshot snapshot,
            int npcKey, out EmpressNativeCombatObservation observation)
        {
            observation = default(EmpressNativeCombatObservation);
            if (snapshot?.PriorityBoss == null) return false;
            var values = snapshot.PriorityBoss.Empresses;
            for (var i = 0; i < values.Count; i++)
            {
                var candidate = values[i];
                if (candidate.NpcKey != npcKey || !candidate.Known) continue;
        ...
        private static int NextAttack(bool second, bool expertSchedule,
            int index)
        {
            var table = !second ? PhaseOneAttacks : expertSchedule ?
                PhaseTwoExpertAttacks : PhaseTwoClassicAttacks;
            return table[index % table.Length];
        }

        private static int PreviousAttack(bool second, bool expertSchedule,
            int currentIndex)
        {
            var table = !second ? PhaseOneAttacks : expertSchedule ?
                PhaseTwoExpertAttacks : PhaseTwoClassicAttacks;
            return table[(currentIndex - 1) % table.Length];
        }

        private static bool MatchesPreviousAttack(int state, bool second,
            bool expertSchedule, int currentIndex)
        {
            var scheduled = PreviousAttack(second, expertSchedule,
                currentIndex);
        ...
        private static string AttackName(int state)
        {
            switch (state)
            {
                ...
                case 6: return "sun-dance";
                case 7: return "lance-wall";
                case 8:
                case 9: return "horizontal-dash";
                case 11: return "predictive-lances";
                case 12: return "spiral-bolts";
                default: return "unknown-attack-" + state;
            }
        }

        private BossDecision Departing(CombatSnapshot snapshot,
            TargetSnapshot target, bool enraged)
        {
            var result = Decision(snapshot, target,
                (enraged ? "day-rage-" : "night-") +
                "native-departure", BossPattern.EllipseOrbit, 760, -190,
                0, 0, false, false, false, 180, false);
            result.Directive.UseExplicitMovement = true;
            result.Directive.ForceContinuousMovement = false;
            result.Directive.RequestControlReturn = true;
            result.Directive.ControlReturnReason =
                "Empress of Light entered her native departure state";
            return result;
        }
        ...
        {
            var result = Decision(snapshot, target, phase,
                BossPattern.EllipseOrbit, 760, -190, 0, 0, false, false,
                false, 180, false);
            result.Directive.UseExplicitMovement = true;
            result.Directive.ForceContinuousMovement = false;
            result.Directive.RequestControlReturn = true;
            ...
            TargetSnapshot target, string reason)
        {
            var result = Decision(snapshot, target,
                "unmodeled-native-threat", BossPattern.ProjectileLanes,
                760, -190, 0, 0, false, false, false, 180, false);
            result.Directive.UseExplicitMovement = true;
            result.Directive.OwnsMovementClosure = true;
            result.Directive.ForceContinuousMovement = false;
            result.Directive.HoldNeutralControls = true;
